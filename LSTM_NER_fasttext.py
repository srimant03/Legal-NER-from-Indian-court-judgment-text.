...
torch.save(model.state_dict(), 'LSTM_NER_fasttext.pt')
...
plt.tight_layout()
plt.savefig('LSTM_NER_fasttext.pdf')
plt.show()



model = LSTMModel(embedding_dim, hidden_dim, output_dim)
model.load_state_dict(torch.load('LSTM_NER_fasttext.pt'))
model.eval()
...
#calculate label-wise f1 score of each label in the test set
from sklearn.metrics import classification_report

# Calculate classification report
report = classification_report(test_targets, test_predictions, output_dict=True)

# Access label-wise macro-F1 scores
label_wise_f1 = {label: metrics['f1-score'] for label, metrics in report.items() if label.isdigit()}

for label, f1 in label_wise_f1.items():
    print(f'Label {label}: Macro-F1 = {f1:.4f}')
    #print key of label from tag_to_ix dictionary
    for key, value in tag_to_ix.items():
        if value == int(label):
            print(f'Label {label}: {key}')
            break

#create a bar plot for each label and its macro-f1 scor
