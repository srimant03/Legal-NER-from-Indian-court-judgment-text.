...
def calculate_label_wise_f1(model, dataset):
    ...
    for sentence, tags in dataset:
        ...
        for pred_tag, true_tag in zip(predicted_tags, tags):
            label_predictions[true_tag].append(pred_tag)
            label_targets[true_tag].append(true_tag)
    
    label_f1_scores = {}
    for label in label_predictions.keys():
        label_predictions_list = label_predictions[label]
        label_targets_list = label_targets[label]
        f1 = f1_score(label_targets_list, label_predictions_list, average='macro')
        label_f1_scores[label] = f1
    
    return label_f1_scores
