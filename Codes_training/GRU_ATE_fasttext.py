    ...
    with torch.no_grad():
        for tokens, labels in val_data:
            ...
            for token_emb, label_idx in zip(token_embeddings, label_indices):
                ...
                label_idx = torch.tensor(label_idx).long()
                output = model(token_emb)
                val_loss = criterion(output, label_idx)
...
torch.save(model.state_dict(), 'gru_ate_fasttext.pt')
...
plt.tight_layout()
plt.savefig('gru_ate_fasttext.pdf')
plt.show()

#save the plot as a png file

model = GRUModel(embedding_dim, hidden_dim, output_dim)
model.load_state_dict(torch.load('gru_ate_fasttext.pt'))
model.eval()
